...
Run generation now includes `lastRun.objective` (lane target, perfect-action target, optional element attunement, bonus spores, and branching chain metadata). Objective chains branch automatically (`origin` -> `ascend` on success, `recover` on failure) so the next run responds to your last objective result. Encounter tuning now adapts per branch and exposes `encounter.branchDirective` in run payloads so UI can surface pressure/comeback context lane-by-lane. Session receipts include `rewards.objectiveCompleted`, `rewards.objectiveProgress`, and `rewards.objectiveBonusSpores` so UI can show objective progress in real time.
